...
| **Portal registry** | Server-authoritative list from `ZDOMan.GetPortalList()`, pushed to clients on join and on change. **Each record carries that portal's clearance mask**, not just name, id and position — see the row below for why. Game 1.0 buckets portals by sector, so `GetPortals()` now returns a dictionary and `GetPortalList()` flattens it into a fresh list per call — a copy, unlike the live list the old `GetPortals()` handed back (§12). |
| **Where the target lives** | Our own `stave_dest` key on the portal's ZDO, holding the destination's permanent `stave_pid` rather than a ZDOID — the game renumbers every ZDO on load, which is what the abandoned `stave_target` key got wrong and why the server strips it on sight (§12). A prefix on `TeleportWorld.Teleport` prefers it over the vanilla destination. **Not** the vanilla `ConnectionType.Portal` connection: the server rebuilds those from tag matches every 5 seconds and would erase a one-way target almost immediately (§12). Leaving that pass alone is also what gives §5's vanilla-tag fallback for free — a portal we never re-aimed still pairs the way it always did. |
...
- **`ZDOMan.RequestZDO` reaches a ZDO outside your area.** It routes to the server's
  `RPC_RequestZDO`, which calls `ForceSendZDO` for that peer — so a client can refresh any ZDO it knows
  the id of, however far away. Both the portal registry and the left-behind-body sweep rely on it.
- **`ZNetScene.CreateDestroyObjects` culls by ZDO position, not by the drawn transform.** It collects
  what is near the reference position with `FindSectorObjects` and hands the rest to `RemoveObjects`.
  So the way to remove a stale character is to correct the ZDO the client holds, not to move or hide
  the object — refresh it and the game removes it.
